...
namespace VerGen.Utility
{
    ...
    public class TypeHelper
    {
        ...
        public string Escape(TypeUsage typeUsage)
        {
            ...
            {
                Type clrType = UnderlyingClrType(typeUsage.EdmType);
        ...
        {
            return typeUsage == null ? null : GetTypeName(typeUsage.EdmType, IsNullable(typeUsage), modelNamespace: null);
        }
        ...
        {
            return typeUsage == null ? null : GetTypeName(typeUsage.EdmType, IsNullable(typeUsage), modelNamespace);
        }
        ...
        private string GetTypeName(EdmType edmType, bool? isNullable, string modelNamespace)
        {
            ...
            var externalTypeName = Escape(edmType.MetadataProperties
                .Where(p => p.Name == ExternalTypeNameAttributeName)
                .Select(p => (string)p.Value)
                .FirstOrDefault());
            var typeName = string.IsNullOrEmpty(externalTypeName) ?
                (modelNamespace != null && edmType.NamespaceName != modelNamespace ?
                 CreateFullName(EscapeNamespace(edmType.NamespaceName), Escape(edmType)) :
                 Escape(edmType)) : externalTypeName;
